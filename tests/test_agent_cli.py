...
def test_legacy_invocations_map_to_subcommands():
    assert cli._normalize_legacy_args(["revenue by plan"]) == ["ask", "revenue by plan"]
    assert cli._normalize_legacy_args(
        ["--retrieval-only", "revenue by plan", "-k", "3"]
    ) == ["retrieve", "revenue by plan", "-k", "3"]
    assert cli._normalize_legacy_args(["retrieve", "q"]) == ["retrieve", "q"]
    assert cli._normalize_legacy_args(["--help"]) == ["--help"]


def test_parser_exposes_retrieval_and_semantic_options():
    args = cli.build_parser().parse_args([
        "retrieve", "MRR by region", "--config", "rrf_hybrid", "--semantic-layer", "-k", "8",
    ])
    assert args.command == "retrieve"
    assert args.config == "rrf_hybrid"
...
def test_retrieve_lexical_json_is_real_typed_pipeline(tmp_path, capsys):
    db = build_saas(tmp_path / "saas.db")
    capsys.readouterr()
    rc = cli.main([
        "retrieve", "revenue by plan", "--db", str(db),
        "--config", "lexical_baseline", "--json",
    ])
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["config_name"] == "lexical_baseline"
...
def test_value_config_reports_missing_index_before_query(tmp_path, monkeypatch, capsys):
    ...
    rc = cli.main([
        "retrieve", "tickets for Globex Corporation", "--db", str(db),
        "--config", "dense_value", "--es-url", "http://es.test:9200",
    ])
    assert rc == 2
