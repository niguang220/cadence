...
def test_config_roundtrip():
    for c in (RetrievalConfig.default(), RetrievalConfig.rrf_hybrid(),
...
def _full_result():
    ...
    lexical_only = TableCandidate("account", {}, 0.01, 1)
    rrf = TableCandidate("account", {"lexical": ctr}, 0.016, 1)
    return RetrievalResult(
        config_name="rrf_hybrid", signals=[sig], candidates=[lexical_only, rrf],
...
def test_result_roundtrip_including_events():
    r = _full_result()
    back = deserialize_result(serialize_result(r))
    assert back == r
    assert back.candidates[0].fusion_score == 0.01
